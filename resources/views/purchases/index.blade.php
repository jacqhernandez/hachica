...
<div class="panel-heading" id="panel-heading">
	<div>
		Purchases <?php if (!empty($label)) { echo $label; }?>
		<span class="pull-right">	<a href="{{ route('purchases.create') }}" class="btn btn-primary pull-right" style="font-size: 10px; font-weight: 600; margin-left: 20px;"><span class="glyphicon glyphicon-plus"></span></a></span>
	</div>
	<div class="pull-right " id="purchases-dates">
		<form>
...
<div class="panel-body purchases-details">
	<div>
		Total Purchases: &#8369;{{ $total_purchases }}<br>
		Total Transactions: {{ $purchases->count() }}
	</div>		
</div>
	...
	<table class="table table-hover sortable"> 
		...
		<tbody>
			@foreach ($purchases as $purchase)
				<tr>
					<td>{{ Carbon\Carbon::parse($purchase->purchase_date)->format('F d, Y (l)') }}</td>
					<td>{{ $purchase->supplier }}</td>
					<td class="tr-number">&#8369;{{ $purchase->total_amount }}</td>
					<td></td><td></td><td></td><td></td>	
					<td>
						<div id="show"><a href="{{ route('purchases.show', [$purchase->id]) }}" class="btn btn-info btn-main-info">View</a></div>
					</td>
				</tr>
			@endforeach
		</tbody> 
